...
public class EpidemicOracleRouter extends ActiveRouter {
	...
	private void sendMessageToConnected(Message m) {
		...
		//BINGO!!!!!
		if(host.getAddress()%10 == 0) {
			
		}
		else {
			for (Connection c : getConnections()) {
				int fl = 0;
				if(c.isReadyForTransfer() && c.getMessage()!=null && c.getOtherNode(host).getAddress()%10 == 0) {
					fl = 1;
					transferAborted(c);
					c.abortTransfer();
				}
				if(fl==0 && c.getMessage()!=null && c.getMessage().getHops()!=null) {
					int size = c.getMessage().getHops().size();
					if(size >= 2) {
						del[c.getMessage().getHops().get(size-2).getAddress()][getHost().getAddress()]++;
					}
					//System.out.println(c.getMessage().getHops()+ " " + getHost());
					
					}
				if (fl==0 &&c.isReadyForTransfer() && c.startTransfer(host, m) == RCV_OK) {
	...
	public void removeDeliveredMessage(String id) {
		if (this.hasMessage(id)) {
			...
			//System.out.println(id + "--->" + this); S.Saha check
