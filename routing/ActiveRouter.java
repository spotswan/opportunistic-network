...
public abstract class ActiveRouter extends MessageRouter {
	...
	public static int del[][] = new int[N][N];
...
//		}
	...
	public void update() {		
		super.update();
		int police = getHost().police;
		int uppergh = getHost().uppergh;
		int lowergh = getHost().lowergh;
		...
		for (int i=0; i<this.sendingConnections.size(); ) {
			...
			//System.out.println(con.getOtherNode(getHost()) + " " + con);
			/* finalize ready transfers */
			//int size = con.getMessage().getHops().size();
			int fl = 0;
			
			if(con.getMessage()!= null && con.getMessage().getTo().getAddress() != con.getOtherNode(getHost()).getAddress()) {
				int temp = con.getOtherNode(getHost()).getAddress();
				if(getHost().gH[temp] == 1) {
					//System.out.println(con.getMessage().getHops()+ " " + getHost());
					transferAborted(con);
					con.abortTransfer();
					fl = 1;
				}
		...
		if (fl == 0 && con.isMessageTransferred()) {
//			if(con.isMessageTransferred()) {
				//to create black hole node S.Saha
				boolean test = false;
				int other = con.getOtherNode(getHost()).getAddress();
				//if the sender is the host
//				if(con.getMessage() != null) test = con.getMessage().getFrom().getAddress() == getHost().getAddress();
				//**HC**	
				if(test == false && (getHost().getAddress() >= lowergh) && (getHost().getAddress() <= uppergh) && (other <= 45 || other >= 50)) {
//					
//					//getHost().deleteMessage(con.getMessage().getId(), true);
//					//removeCurrent = true;
//					
				}
				else if (con.getMessage() != null) {
//				if(con.getMessage() != null) {
					
//					if(con.getMessage().getHops()!=null) {
//						int size = con.getMessage().getHops().size();
//						if(size >= 3) {
//							del[con.getMessage().getHops().get(size-3).getAddress()][getHost().getAddress()]++;
//						}
						//System.out.println(con.getMessage().getHops()+ " " + getHost());
						
//						}
					//System.out.println(con.getMessage().getUniqueId());
					
					int key = con.getMessage().getUniqueId(); //Message Key
					int otherNode = con.getOtherNode(getHost()).getAddress(); //Recieving Node value
					//If intermediate Node
					if(getHost().mesTran.containsKey(key)) {
						int arr[] = new int[2];
						arr = getHost().mesTran.get(key);
						//int otherNode = con.getOtherNode(getHost()).getAddress();
						arr[1] = otherNode;

						//getHost().mesTran.put(key, getHost().mesTran.get(key) + 1);
					}
					//If source Node
					else {
						int arr[] = new int[2];
						arr[0] = getHost().getAddress();
						arr[1] = otherNode;
						
						getHost().mesTran.put(key, arr);
					}
					if(!con.getOtherNode(getHost()).mesTran.containsKey(key)) {
						int arr[] = new int[2];
						arr[0] = getHost().getAddress();
						
						con.getOtherNode(getHost()).mesTran.put(key, arr);
						
					}
					//getHost().mesTran[con.getMessage().getUniqueId()]
					transferDone(con);
...
//					if(con.getMessage()!=null) {
//					int size = con.getMessage().getHops().size()-1;
//					System.out.println(con.getMessage().getHops().get(size));		
//					}
