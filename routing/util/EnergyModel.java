...
public class EnergyModel implements ModuleCommunicationListener {
	/**
	 * Initial units of energy -setting id ({@value}). Can be either a single value,
	 * or a range of two values. In the latter case, the used value is a uniformly
	 * distributed random value between the two values.
	 */
	public static final String INIT_ENERGY_S = "30";

	/** Energy usage per scanning (device discovery) -setting id ({@value}). */
	public static final String SCAN_ENERGY_S = "40";

	/**
	 * Energy usage per scanning (device discovery) response -setting id ({@value}).
	 */
	
	//Nothing called Recharge Energy or Recharge Energy Random
	//No Recieve Energy 
	public static final String SCAN_RSP_ENERGY_S = "30";

	/** Energy usage per second when sending -setting id ({@value}). */
	public static final String TRANSMIT_ENERGY_S = "10";

	/** Base energy usage per second -setting id ({@value}). */
	public static final String BASE_ENERGY_S = "10";

	/**
	 * Energy update warmup period -setting id ({@value}). Defines the simulation
	 * time after which the energy level starts to decrease due to scanning,
	 * transmissions, etc. Default value = 0. If value of "-1" is defined, uses the
	 * value from the report warmup setting {@link report.Report#WARMUP_S} from the
	 * namespace {@value report.Report#REPORT_NS}.
	 */
	public static final String WARMUP_S = "-1";

	/**
	 * {@link ModuleCommunicationBus} identifier for the "current amount of energy
	 * left" variable. Value type: double
	 */
	public static final String ENERGY_VALUE_ID = "30";
	...
	public EnergyModel(Settings s) {
		...
		if (this.initEnergy.length != 1 && this.initEnergy.length != 2) {
			throw new SettingsError(
					INIT_ENERGY_S + " setting must have " + "either a single value or two comma separated values");
		}
	...
	/**
	 * Updates the current energy so that the given amount is reduced from it. If
	 * the energy level goes below zero, sets the level to zero. Does nothing if the
	 * warmup time has not passed.
	 * 
	...
	/**
	 * Reduces the energy reserve for the amount that is used by sending data and
	 * scanning for the other nodes.
	 */
